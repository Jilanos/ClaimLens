...
- request-AC3 -> `item_099_remplacer_l_avatar_de_navigation_par_l_embleme_paul_mondou`. Proof: the link carries `aria-label="Visit paulmondou.fr"`, `target="_blank"`, `rel="noopener noreferrer"`, an empty `alt` on the decorative image, and a `:focus-visible` outline matching the navigation style; `Logout`, the e-mail and the existing links are untouched. Covered by `::test_the_parent_link_opens_safely_and_announces_its_destination`.
