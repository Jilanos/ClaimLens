## item_054_configure_supadata_first_deployment - Configure Supadata-first deployment
> From version: 1.0.0
> Schema version: 1.0
> Status: Done
> Understanding: 90%
...
# AC Traceability
- request-AC1 -> This backlog slice. Proof: AC1: `docs/deployment-paulmondou-infra.md` sets `CLAIMLENS_TRANSCRIPT_PROVIDER_ORDER=supadata,youtube` in the ClaimLens Compose service; the deployed web entrypoint consumes that environment variable. `config/claimlens.example.toml` remains local-first with `youtube` as its default.
- request-AC2 -> This backlog slice. Proof: AC2: With a valid enabled Supadata key, a run fetches captions via `supadata-native` without ever invoking the classic `youtube_transcript_api` path first.
- request-AC3 -> This backlog slice. Proof: AC3: `youtube` remains available strictly as an explicit, configurable secondary fallback and is only attempted after all eligible Supadata keys fail to return native captions.
...
# Links
- Product brief(s): `prod_009_configure_supadata_first_deployment`
...
# AI Context
- Summary: Configure Supadata-first deployment
- Keywords: backlog-groom, request, configure supadata-first deployment, bounded slice
- Use when: Use when implementing or reviewing the delivery slice for Configure Supadata-first deployment.
